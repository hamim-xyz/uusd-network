...
router.get('/bnb/balance/:telegramId', async (req, res) => {
  try {
    ...
    let balances = typeof rows[0].balances === 'string' ? JSON.parse(rows[0].balances) : (rows[0].balances || {});
    const prev = Number(balances.BNB || 0);

    if (onChainNum > prev + 1e-12) {
      const deposited = onChainNum - prev;
      balances.BNB = onChainNum;
      await query('UPDATE wallets SET balances = ? WHERE telegram_id = ?', [
        JSON.stringify(balances),
        req.params.telegramId,
      ]);
      const { generateId } = await import('../utils/crypto.js');
...
router.post('/bnb/withdraw', requireTelegramUser, rejectIfBlocked, async (req: TelegramRequest, res) => {
  try {
    const telegramId = req.telegramId || req.body.telegramId;
    const { toAddress, amount, pin } = req.body;
    if (!telegramId || !toAddress || amount === undefined) {
      return res.status(400).json({ error: 'telegramId, toAddress, amount required' });
    }
    const amt = Number(amount);
    if (!(amt > 0)) return res.status(400).json({ error: 'Invalid amount' });

    const sec: any[] = await query('SELECT pin_hash, salt FROM user_security WHERE telegram_id = ?', [telegramId]);
    if (sec.length) {
      if (!pin) return res.status(400).json({ error: 'PIN required' });
      const { hashPin } = await import('../utils/crypto.js');
      if (hashPin(String(pin), sec[0].salt) !== sec[0].pin_hash) {
    ...
    const newBal = Number(await getBnbBalance(w.address));
...
router.get('/onchain/history/:telegramId', async (req, res) => {
  try {
