...
router.post('/referrals/bind', async (req, res) => {
  try {
    ...
    if (!referrerTelegramId || !referredTelegramId) {
      return res.status(400).json({ error: 'referrer and referred required' });
    }
    if (referrerTelegramId === referredTelegramId) {
      return res.status(400).json({ error: 'Cannot refer yourself' });
    }
    const existing: any[] = await query('SELECT id FROM referrals WHERE referred_telegram_id = ? LIMIT 1', [referredTelegramId]);
    if (existing.length) return res.json({ success: true, alreadyBound: true });
    const id = generateId('ref_');
    await query('INSERT INTO referrals (id, referrer_telegram_id, referred_telegram_id, reward_given) VALUES (?, ?, ?, 0)',
...
router.post('/create', requireTelegramUser, rejectIfBlocked, async (req: TelegramRequest, res) => {
  ...
  try {
    ...
    await conn.execute(`INSERT INTO users (telegram_id, address, first_name, username, photo_url) VALUES (?, ?, ?, ?, ?)`,
    ...
    await conn.commit();
    res.json({
      wallet: { telegramId, address, availableBalance: 0, lockedBalance: 0,
        balances: { UUSD: 0, BNB: 0 }, depositEnabled: true, blocked: false, createdAt: new Date().toISOString() },
    });
  } catch (e: any) {
    await conn.rollback();
    res.status(500).json({ error: e.message });
...
router.post('/transfer', requireTelegramUser, rejectIfBlocked, async (req: TelegramRequest, res) => {
  ...
  try {
    ...
    const [secRows]: any = await conn.execute('SELECT pin_hash, salt FROM user_security WHERE telegram_id = ? LIMIT 1', [senderTelegramId]);
    if (secRows.length) {
    ...
    const [senders]: any = await conn.execute('SELECT * FROM wallets WHERE telegram_id = ? FOR UPDATE', [senderTelegramId]);
    if (!senders.length || senders[0].blocked) {
      await conn.rollback();
      return res.status(senders.length ? 403 : 404).json({ error: senders.length ? 'Blocked' : 'Wallet not found' });
    }
    let senderBalances: any = {};
    try { senderBalances = typeof senders[0].balances === 'string' ? JSON.parse(senders[0].balances) : (senders[0].balances || {}); } catch {}
    if (Number(senderBalances[symbol] || 0) < amount) {
      await conn.rollback();
      return res.status(400).json({ error: 'Insufficient funds' });
    }
    const [recWallets]: any = await conn.execute('SELECT * FROM wallets WHERE telegram_id = ? FOR UPDATE', [recipient.telegram_id]);
    if (!recWallets.length || recWallets[0].blocked) {
      await conn.rollback();
      return res.status(403).json({ error: 'Recipient unavailable' });
    }
    let recBalances: any = {};
    try { recBalances = typeof recWallets[0].balances === 'string' ? JSON.parse(recWallets[0].balances) : (recWallets[0].balances || {}); } catch {}
...
router.get('/:telegramId', async (req, res) => {
  try {
    ...
    let balances = {};
    try { balances = typeof wallet.balances === 'string' ? JSON.parse(wallet.balances) : (wallet.balances || {}); } catch {}
    const activities: any[] = await query(
      `SELECT id, type, amount, symbol, status, to_address as toAddress, to_name as toName,
       ...
       FROM activities WHERE telegram_id = ? ORDER BY created_at DESC LIMIT 100`, [telegramId]);
    res.json({
      wallet: {
        telegramId: wallet.telegram_id, address: wallet.address,
        availableBalance: Number(wallet.available_balance), lockedBalance: Number(wallet.locked_balance),
        balances, depositEnabled: !!wallet.deposit_enabled, blocked: !!wallet.blocked, createdAt: wallet.created_at,
      },
      activities, needsCreation: false,
    });
  } catch (e: any) {
    res.status(500).json({ error: e.message });
  }
